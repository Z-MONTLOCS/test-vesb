...
USERNAME = 'zyrivic'   # Your DBC username here
PASSWORD = '5RL:6dRdfadS#Hc'   # Your DBC password here
CAPTCHA_URL = 'https://aplicaciones.adres.gov.co/bdua_internet/Pages/ConsultarAfiliadoWeb.aspx'
    ...
    with webdriver.Chrome() as driver:  #using Chrome
        ...
        try:
            element: WebElement = WebDriverWait(driver, 10).until(
                EC.presence_of_element_located((By.ID, "Capcha_CaptchaImageUP"))
            )
        except:
            print('Not found')
            return
